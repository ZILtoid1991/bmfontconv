...
// Sean Barrett 2014
// 
// D conversion by Laszlo Szeremi
//
...
int packRects(stbrp_context* context, stbrp_rect[] rects) {
	return stbrp_pack_rects(context, rects.ptr, cast(int)rects.length);
}
...
int stbrp_pack_rects (stbrp_context* context, stbrp_rect* rects, int num_rects) {
	...
	STBRP_SORT(rects, num_rects, rects[0].sizeof, &rect_height_compare);
	...
	// unsort
	STBRP_SORT(rects, num_rects, rects[0].sizeof, &rect_original_order);
...
void stbrp_setup_heuristic (stbrp_context *context, int heuristic) {
	switch (context.init_mode) {
		case STBRP__INIT_skyline:
			assert(heuristic == STBRP_HEURISTIC.Skyline_BL_sortHeight || heuristic == STBRP_HEURISTIC.Skyline_BF_sortHeight);
...
stbrp__findresult stbrp__skyline_find_best_pos(stbrp_context* c, int width, int height)
{
	...
	stbrp_node** prev, best;
	stbrp_node* node, tail;

	// align to multiple of c.align
	width = (width + c._align - 1);
	width -= width % c._align;
	assert(width % c._align == 0);
...
extern(C) int rect_height_compare(const void* a, const void* b){
...
extern(C) int rect_original_order(const void* a, const void* b)
{
   const stbrp_rect* p = cast(const stbrp_rect*)a;
   const stbrp_rect* q = cast(const stbrp_rect*)b;
